        ...
        if (ZKF.compareTo(BigDecimal.ZERO) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7356"))).setScale(0, BigDecimal.ROUND_DOWN);
            //  Neu 2017
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3678"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  Neu 2017
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
        ...
        if (X.compareTo(GFB.add(ZAHL1)) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13770")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("54058")) === -1) {
                    Y = (X.subtract(new BigDecimal("13769"))).divide(ZAHL10000, 6, BigDecimal.ROUND_DOWN);
                    RW = Y.multiply(new BigDecimal("223.76"));
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("939.57"))).setScale(0, BigDecimal.ROUND_DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("256304")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8475.44"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("16164.53"))).setScale(0, BigDecimal.ROUND_DOWN);
                    }
                }
            }
        }
