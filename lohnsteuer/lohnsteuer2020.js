        ...
        if (ZKF.compareTo(BigDecimal.ZERO) === 1)         //  ZKF > 0
{
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("7812"))).setScale(0, BigDecimal.ROUND_DOWN);
            //  Neu 2020
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3906"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  Neu 2020
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
...
{
                    Y = (X.subtract(new BigDecimal("14532"))).divide(ZAHL10000, 6, BigDecimal.ROUND_DOWN);
                    RW = Y.multiply(new BigDecimal("212.02"));
                    //  neu seit 2020
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("972.79"))).setScale(0, BigDecimal.ROUND_DOWN);
                    //  neu seit 2020
                } else {
                    if (X.compareTo(new BigDecimal("270501")) === -1)                     //  neu seit 2020
{
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8963.74"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  neu seit 2020
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("17078.74"))).setScale(0, BigDecimal.ROUND_DOWN);
                        //  neu seit 2020
                    }
                }
            }
