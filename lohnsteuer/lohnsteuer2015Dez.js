...
module.exports = function Lohnsteuer2015DezemberBig(args) {
    ...
    var ZRE4VPM = BigDecimal.ZERO; // BigDecimal
        ...
        if (ZKF.compareTo(BigDecimal.ZERO) === 1)         //  ZKF > 0
{
    ...
    function MZTABFBN() {
        ...
        if (STKL === 1) {
            SAP = new BigDecimal("36");
            KFB = (ZKF.multiply(new BigDecimal("7152"))).setScale(0, BigDecimal.ROUND_DOWN);
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3576"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        if (STKL === 5) {
                            SAP = new BigDecimal("36");
                            KFB = BigDecimal.ZERO;
        ...
        if (KRV > 1)         //  &lt; = < &gt; = >
{
    ...
    function MLST1215() {
        if (LZZ > 1) {
            JW = (JWLSTN.subtract(new BigDecimal("11").multiply((JWLSTA.subtract(JWLSTN))))).setScale(0, BigDecimal.ROUND_DOWN);
            if (JW.compareTo(BigDecimal.ZERO) === -1)             //  JW < 0
{
            ...
            LSTLZZ = ANTEIL1;
            JW = (JWSOLZN.subtract(new BigDecimal("11").multiply(JWSOLZA.subtract(JWSOLZN)))).setScale(0, BigDecimal.ROUND_DOWN);
            if (JW.compareTo(BigDecimal.ZERO) === -1)             //  JW < 0
{
                ANTEIL1 = BigDecimal.ZERO;
            } else {
            ...
            SOLZLZZ = ANTEIL1;
            JW = (JWBKN.subtract(new BigDecimal("11").multiply(JWBKA.subtract(JWBKN)))).setScale(0, BigDecimal.ROUND_DOWN);
            if (JW.compareTo(BigDecimal.ZERO) === -1)             //  JW < 0
{
                ANTEIL1 = BigDecimal.ZERO;
            } else {
        ...
        if (X.compareTo(new BigDecimal("8355")) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13470")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("52882")) === -1) {
                    ...
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("971"))).setScale(0, BigDecimal.ROUND_DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("250731")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8239"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("15761"))).setScale(0, BigDecimal.ROUND_DOWN);
                    }
                }
            }
        }
        ...
        if (X.compareTo(new BigDecimal("8473")) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13470")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("52882")) === -1) {
                    ...
                    RW = RW.add(new BigDecimal("2397"));
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("948.68"))).setScale(0, BigDecimal.ROUND_DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("250731")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8261.29"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("15783.19"))).setScale(0, BigDecimal.ROUND_DOWN);
                    }
                }
            }
        }
