...
module.exports = function Lohnsteuer2009Big(args) {
    ...
    function MZTABFB() {
        ...
        if (STKL === 1) {
            ...
            KFB = (ZKF.multiply(new BigDecimal("6024"))).setScale(0, BigDecimal.ROUND_DOWN);
        } else {
            if (STKL === 2) {
                ...
                if (STKL === 3) {
                    ...
                    if (STKL === 4) {
                        SAP = new BigDecimal("36");
                        KFB = (ZKF.multiply(new BigDecimal("3012"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        KFB = BigDecimal.ZERO;
                    }
                }
    ...
    function UPEVP() {
        ...
        if (KRV > 0) {
            ...
            VSP1 = (ZRE4VP.multiply(new BigDecimal("0.36"))).setScale(2, BigDecimal.ROUND_DOWN);
    ...
    function MST5_6() {
        ...
        if (ZZX.compareTo(new BigDecimal("26276")) === 1) {
            ...
            if (ZZX.compareTo(new BigDecimal("200320")) === 1) {
                ST = (ST.add((new BigDecimal("200320").subtract(new BigDecimal("26276"))).multiply(new BigDecimal("0.42")))).setScale(0, BigDecimal.ROUND_DOWN);
                ST = (ST.add((ZZX.subtract(new BigDecimal("200320"))).multiply(new BigDecimal("0.45")))).setScale(0, BigDecimal.ROUND_DOWN);
            } else {
                ST = (ST.add((ZZX.subtract(new BigDecimal("26276"))).multiply(new BigDecimal("0.42")))).setScale(0, BigDecimal.ROUND_DOWN);
            }
        } else {
            ZX = ZZX;
            UP5_6();
            if (ZZX.compareTo(new BigDecimal("9225")) === 1) {
                ...
                UP5_6();
                HOCH = (ST.add((ZZX.subtract(new BigDecimal("9225"))).multiply(new BigDecimal("0.42")))).setScale(0, BigDecimal.ROUND_DOWN);
        ...
        if (X.compareTo(new BigDecimal("7835")) === -1) {
            ...
            if (X.compareTo(new BigDecimal("13140")) === -1) {
                ...
                if (X.compareTo(new BigDecimal("52552")) === -1) {
                    ...
                    RW = RW.multiply(Y);
                    ST = (RW.add(new BigDecimal("1007"))).setScale(0, BigDecimal.ROUND_DOWN);
                } else {
                    if (X.compareTo(new BigDecimal("250401")) === -1) {
                        ST = ((X.multiply(new BigDecimal("0.42"))).subtract(new BigDecimal("8064"))).setScale(0, BigDecimal.ROUND_DOWN);
                    } else {
                        ST = ((X.multiply(new BigDecimal("0.45"))).subtract(new BigDecimal("15576"))).setScale(0, BigDecimal.ROUND_DOWN);
                    }
                }
            }
        }
